...
const Checkout = (props) => {
    ...
    useEffect(() => {
        const getCartData = async () => {
            ...
            if(userData != null){
                axios.get(`${baseUrl}wc/v3/customers/${userData.user_id}?consumer_key=${consumer_key}&consumer_secret=${consumer_secret}`)
                .then(res=>{
                    const {email, last_name, first_name, username, billing} = res.data
                    console.log(res.data);
                    setAddressData(
                        {...addressData, ...billing, ['email']: email, username, ['last_name']:last_name, ['first_name']: first_name, ['sign']: "true"}
                    )
                })
                .catch(err=>{
                    console.log(err);
    ...
    const handleNext = async () => {
        ...
        if(activeStep === steps.length -1 ){
            ...
            setLoading(true);
            axios.post(`${baseUrl}wc/v3/orders?consumer_key=${consumer_key}&consumer_secret=${consumer_secret}`,payload)
            .then(res=>{
            ...
            if(localStorage.getItem('__wp') == null){
                if(addressData.createAccount){
                    const {email, first_name, last_name, username, password, phone, address_1, address_2, city, state, postcode, country} = addressData;
                    if(username && password){
                        axios.post(`${baseUrl}wc/v3/customers?consumer_key=${consumer_key}&consumer_secret=${consumer_secret}`,{
                            email,
                            first_name,
                            last_name,
                            username,
                        ...
                        setCreateAccountInfo('Username and password was not given properly :(');
                    }
                }
            }
        }
        ...
        <Grid item xs={7} sm={8} md={10} style={{ paddingLeft: '15px' }}>
            <Container component="main"  sx={{ mb: 4 }}>
                ...
                <>
                    ...
                    <Typography variant="subtitle1">
                    Check your network connection, if that is okay try to resubmit the order. sorry for the inconvenience.
                    </Typography>
                    {addressData.createAccount && <>
